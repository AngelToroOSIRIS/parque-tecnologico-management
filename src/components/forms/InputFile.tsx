...
const InputFile = ({
  ...
  for (let i = 0; i < selectedExtensions.length; i++) {
    formatsText += `${String(selectedExtensions[i]).toUpperCase()}${
      i === selectedExtensions.length - 1 ? "." : ","
    } `;
  }
  ...
  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!filesInput || (filesInput && filesInput.length === 0)) {
      if (minFiles > 0) {
        toast.error(
          `Debe seleccionar al menos ${minFiles} ${
            minFiles > 1 ? "archivos" : "archivo"
          }`
        );
      }
      setFilesSelected({
    ...
    if (filesInput.length >= minFiles) {
      if (maxFiles) {
        if (filesInput.length > maxFiles) {
          toast.error(
            `Debe seleccionar máximo ${maxFiles} ${
  ...
  return (
    <div className="text-start w-full">
      ...
      <label title={formatsText} htmlFor={name}>
        <div
          className={`w-full h-[48px] bg-[#ffff] transition-all select-none outline-none rounded-full border-2 ${
            filesSelected.valid
              ? "border-green hover:border-dark-green"
      ...
      <p
        className={`font-semibold select-none ${
          filesSelected.valid ? "text-green" : "text-primary"
        } mt-1 mb-[10px]`}
      >
        {filesSelected.valid &&
