...
const Button = ({ route, text, onClick, type, disabled = false }: Props) => {
  ...
  const classbtn = "w-full h-10 border-2 rounded-xl xl:text-md text-base md:h-10 shadow-[rgba(50,50,93,0.25)_0px_6px_12px_-2px,_rgba(0,0,0,0.3)_0px_3px_7px_-3px] hover:font-semibold text-primary hover:bg-primary hover:text-default-white hover:border-primary transition-all"
  return (
    <button
    disabled={disabled}
    className={
